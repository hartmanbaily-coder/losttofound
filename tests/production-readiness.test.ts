...
const readyEnv = {
  PRE_CUSTOMER_RESOURCE_PROFILE: "false",
...
describe("production readiness", () => {
  ...
  it("blocks customer launch while the 4 GiB owner-testing profile is active", () => {
    const report = evaluateProductionReadiness(
      {
        ...readyEnv,
        PRE_CUSTOMER_RESOURCE_PROFILE: "true",
      },
      "2026-06-15T00:00:00.000Z"
    );

    expect(report.ready).toBe(false);
    expect(report.blockers.map((item) => item.id)).toContain("customer-resource-profile");
  });
